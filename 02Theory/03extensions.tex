...
\begin{dfn} Field Extension, Morphism of Extensions, Extension Degree.
    ...
    Let $K$ be a field. 
%    Then a $K$-extension is any of the following equivalent things: 
%    \begin{enumerate}
%        \item A $K$-vector space $(L,\iota)$ where $L$ is also a field.
%        \item $(L,\iota)$ where $L$ is a field and 
%        $\iota : K \to L$ is a morphism of rings.
%    \end{enumerate}
%    To go from $(1)$ to $(2)$, 
%    define the morphism of ring as scalar multiplying $1$,
%    \[\iota : K \to L, k \mapsto k_\iota 1_L\]
%    To go from $(2)$ to $(1)$, define scalar multiplication as left-multiplication,
%    \[\iota : K \to End_\Grp(L), k \mapsto (l \mapsto \iota(k)l)\]
