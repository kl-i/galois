...
In this chapter, 
we show that every polynomial over a field
has all its roots in some finite extension and
there is a \emph{smallest} such extension called 
the polynomial's \emph{splitting field}. 
...
\begin{ex} Degree Bounds Number of Roots. 
    ...
    we did not have to assume $f$ factorises into finitely many factors. 
\end{ex}
...
\begin{lem} Quotienting by an Irreducible Element gives a Field. 
    ...
    In particular, if $f$ were monic, it would be the minimal polynomial of $\bar{X}$.
\end{lem}
\begin{proof}
    Let $\bar{g} \in K[X]/(f)$. 
...
\begin{rmk}
    ...
    the correspondence between galois conjugates and extension morphisms.
\end{rmk}
